    ...
    // get the "full" post data if a post was selected. this may be something to add as a prop in the future for custom use cases.
        ...
        apiFetch({
            ...
            const fullpost = {
                ...
                id: response.id,
                content: decodeEntities(response.content.rendered),
            ...
            // Get ACF fields
            apiFetch({
                path: `/acf/v3/${restBase}/${fullpost.id}`
            }).then(acfResponse => {
                fullpost['acf'] = acfResponse.acf;
            });
            ...
            apiFetch({
                path: `/wp/v2/media/${response.featured_media}`
            }).then(mediaResponse => {

                fullpost['featured_media'] = mediaResponse;
...
PostSelector.propTypes = {
    ...
    sourcePostTypes:PropTypes.array,
